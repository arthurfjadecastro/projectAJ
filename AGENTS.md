...
- Início: confira `git status`, branch e upstream. Sem upstream, registre que o trabalho é local. Com upstream e árvore limpa, execute `git pull --ff-only`. Preserve mudanças locais e resolva a posse dos arquivos antes de reconciliar histórico divergente; use rebase somente de commits não publicados. Nunca use `push --force` nem reescreva histórico publicado.
- Pausa ou troca de máquina: valide; quando commit/push e destino estiverem autorizados, publique e confira o hash com `git ls-remote`. Sem essa configuração, deixe HANDOFF local e registre que outro clone ainda não recebeu o trabalho. Para trabalhar em duas máquinas ao mesmo tempo, envie antes o CURRENT_STATE com os arquivos reservados ou use branches separadas.
...
Antes da descoberta, planejamento de piloto ou escolha de arquitetura, leia `docs/cbl/README.md`; ao usar a pesquisa, consulte `docs/research/README.md` e as páginas apontadas. Preserve o Challenge dos materiais como base e registre ratificações em DECISIONS. Datas propostas, notas analíticas e exemplos não são resultados de campo.

Use CBL (Apple): Engage, Investigate e Act, com documentação e reflexão contínuas. O próximo passo documentado é Guiding Questions. Uma solução depende do processo, dos dados e dos critérios de resultado investigados; usar Astra nesta sessão não determina a arquitetura do cliente.

Separe evidência documental, observação operacional, hipótese e decisão do usuário. Uma pesquisa secundária sustenta perguntas; validação do piloto exige observação e dados reais. `CONTEXT.md` contém apenas o glossário. Registre perguntas e sínteses em `docs/cbl/`; specs/tickets acordados vão para `.scratch/`.
