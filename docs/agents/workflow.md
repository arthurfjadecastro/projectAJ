...
**Conflito ao sincronizar:** primeiro preserve mudanças locais, confira o histórico e a posse dos arquivos. Sem upstream, configure o destino autorizado antes de tentar pull. Com árvore limpa, reconcilie apenas commits não publicados com `git pull --rebase`. Para cada arquivo em conflito:
...
4. Binários: identifique explicitamente qual versão pertence a cada commit e preserve ambas antes de escolher uma base e reaplicar mudanças. Durante rebase, `ours` e `theirs` podem representar lados diferentes do esperado; não escolha pelo nome sem conferir o conteúdo.
...
Para a descoberta e o piloto, leia `docs/cbl/README.md`. Challenge e evidências documentais já existentes precedem uma nova rodada de perguntas. Condições de uso dos clientes e scripts Bash: `docs/ai-kit.md`.
